...
function Timeline(options) {
			...
			d3VizObj.addHook("run_on_each_data_row",
				function(r,i) {
					...
					for(var x=0;x&lt;=obj.date_precision;x++) {
						...
						if(x<obj.date_precision) tindex+="-" ;="" }="" if(typeof="" obj.index[tindex]="=" "undefined")="" {="" obj.index[tindex].push(r.d.i);="" );="" if(this.use_index)="" this.makeindex();="" this.addplaybackinterface="function()" var="" anim="document.getElementById(obj.animatorAttachId);" this="" is="" crap="" --="" will="" improve="" later="" d="document.createElement(&quot;div&quot;);" d.id="timeline_anim_" +obj.id;="" d.classname="timeline_anim" da="document.createElement(&quot;a&quot;);" da.href="#" da.title="Start animation" da.onclick="function()" obj.starttimer();};="" at="document.createTextNode(&quot;\u25B6&quot;);" da.appendchild(at);="" d.appendchild(da);="" ");="" d.appendchild(at);="" obj.pausetimer();};="" obj.speeds="=" speeds="[1,100,200,300,500,1000];" else="" if(speeds.length="">1) {
		...
		if(debug||obj.debug) console.log("Setting view to match "+val+"...");
			...
			if(obj.index.length&gt;0) {
				for(var x in obj.index) {
					...
					if(indexdate<current) {="" for(var="" i="" in="" obj.index[x])="" console.log("#"+obj.data_id_prefix+obj.index[x][i]);="" d3.selectall("#"+obj.data_id_prefix+obj.index[x][i])="" .classed(obj.date_past_class,true)="" .classed(obj.date_future_class,false)="" }="" else="" if(indexdate="=current)" .classed(obj.date_now_class,true)="" if(debug||obj.debug)="" console.log("view="" is="" set");="" updates="" based="" on="" the="" slider="" changing="" this.timelinesliderchange="function()" if(obj.timer_playing)="" if(!obj.timer_paused)="" obj.pausetimer();="" obj.timer_playing="true;" obj.timer_paused="true;" var="" val="obj.slider_index[document.getElementById(&quot;timeline_&quot;+obj.id+&quot;_slider&quot;).value];" obj.settime(val);="" if(window.debug||obj.debug)="" console.log("timeline="" initialized");="" timeline.prototype="new" d3viz(timeline);="" version="" of="" comparedate="" function="" below="" that="" can="" be="" run="" arbitrary,="" unvalidated="" arrays="" datearraytonumber(arr)="" cd="" ;="" cd+="arr[0]?String(&quot;0000&quot;+arr[0]).slice(-4):&quot;0000&quot;;" return="" +cd;="" opposite="" above="" —="" takes="" one="" those="" long="" numbers="" and="" turns="" it="" into="" a="" date="" numbertodatearray(num)="" c="[];" num="String(&quot;00000000000000&quot;+num).slice(-14);" c.push(+num.slice(0,4));="" year="" c.push(+num.slice(4,6));="" month="" c.push(+num.slice(6,8));="" day="" c.push(+num.slice(8,10));="" hour="" c.push(+num.slice(10,12));="" min="" c.push(+num.slice(12,14));="" sec="" c;="" an="" array-based="" time="" object.="" ignores="" zones="" daylight="" savings.="" useful="" only="" very="" specific="" contexts,="" like="" this="" template,="" where="" zone="" aspects="" javascript="" date()="" object="" become="" too="" much="" pain="" neck.="" stores="" dates="" times="" as="" array="" increasing="" specificity,="" i.e.="" [year,month,day,hour,minute,second]="" note:="" uses="" 24="" format="" (so="" 00="" to="" 23),="" months="" are="" by="" their="" common="" numbering="" (1="January)." alex="" wellerstein.="" datetimearray()="" datetime="[1,2,3];" this.setdatetime="function(init)" if(!init.length)="" false;="" datetime[0]="init[0];" datetime[1]="(init.length">1)?init[1]:1; //month
	...
	function adjustDown(index,min,max) {
		while(dateTime[index]<min) {="" datetime[index-1]-="1;" datetime[index]+="max;" }="" returns="" a="" big="" number="" that="" corresponds="" with="" the="" date="" info="" —="" allows="" comparisons="" this.comparedate="function()" var="" cd="dateTime[0];" for(var="" i="1;" i<datetime.length;i++)="" cd+="String(&quot;00&quot;+dateTime[i]).slice(-2);" return="" +cd;="" outputs="" an="" iso8601="" formatted="" (which="" can="" be="" read="" by="" javascript's="" date()="" object)="" this.formatdate="function()" if(typeof="" datetime="=" "undefined")="" false;="" if(!datetime.length)="" dl="dateTime.length;" ;="" iso8601+="dateTime[0];" year="" if(dl="">1) iso8601+="-"+String("00"+Math.max(1,dateTime[1])).slice(-2); //month
    ...
    Coordinator.prototype = {
      ...
      emit: function(evtName, data) {
        ...
        if (cStore[evtName]) {
          ...
          for (var i=0; i<len; i++)="" {="" var="" callback="cStore[evtName][i];" callback(data);="" }="" };="" return="" coordinator;="" })();="" _connect="function(scope)" renderer="scope._renderer;" coordinator="scope._coordinator;" store="scope._store;" coordinator.on('renderpartial',="" renderer.renderpartial,="" renderer);="" coordinator.on('renderall',="" renderer.renderall,="" coordinator.on('extremachange',="" function(data)="" scope._config.onextremachange="" &&="" scope._config.onextremachange({="" min:="" data.min,="" max:="" data.max,="" gradient:="" scope._config['gradient']="" ||="" scope._config['defaultgradient']="" });="" store.setcoordinator(coordinator);="" function="" heatmap()="" config="this._config" =="" util.merge(heatmapconfig,="" arguments[0]="" {});="" this._coordinator="new" coordinator();="" if="" (config['plugin'])="" plugintoload="config[&#39;plugin&#39;];" (!heatmapconfig.plugins[plugintoload])="" throw="" new="" error('plugin="" \''+="" +="" '\'="" not="" found.="" maybe="" it="" was="" registered.');="" else="" plugin="HeatmapConfig.plugins[pluginToLoad];" set="" and="" this._renderer="new" plugin.renderer(config);="" this._store="new" plugin.store(config);="" renderer(config);="" store(config);="" _connect(this);="" @todo:="" add="" api="" documentation="" heatmap.prototype="{" adddata:="" function()="" this._store.adddata.apply(this._store,="" arguments);="" this;="" },="" removedata:="" this._store.removedata="" this._store.removedata.apply(this._store,="" setdata:="" this._store.setdata.apply(this._store,="" setdatamax:="" this._store.setdatamax.apply(this._store,="" setdatamin:="" this._store.setdatamin.apply(this._store,="" configure:="" function(config)="" this._config="Util.merge(this._config," config);="" this._renderer.updateconfig(this._config);="" this._coordinator.emit('renderall',="" this._store._getinternaldata());="" repaint:="" getdata:="" this._store.getdata();="" getdataurl:="" this._renderer.getdataurl();="" getvalueat:="" function(point)="" (this._store.getvalueat)="" this._store.getvalueat(point);="" (this._renderer.getvalueat)="" this._renderer.getvalueat(point);="" null;="" heatmap;="" core="" heatmapfactory="{" create:="" heatmap(config);="" register:="" function(pluginkey,="" plugin)="" heatmapconfig.plugins[pluginkey]="plugin;" heatmapfactory;="" load="" data_paths.js="" svgpath(options)="" obj="this;" obj.loadoptions(options);="" this.type="viz" ;="" d3vizobj.viz.push(obj);="" obj.id="d3VizObj.viz.length-1;" if(typeof="" obj.target="=&quot;undefined&quot;)" obj.debug="=&quot;undefined&quot;)" obj.svg="=&quot;undefined&quot;)" obj.pathgroup="=&quot;undefined&quot;)" obj.interpolate="=&quot;undefined&quot;)" obj.autosort="=&quot;undefined&quot;)" obj.linedata="[];" obj.linegroup="[];" obj.group_id="=&quot;undefined&quot;)" +obj.id;="" obj.path_class="=&quot;undefined&quot;)" obj.target_class="=" "pathdata";="" obj.caption_id="=" "undefined")="" this.processdata="function" (r)="" obj.data="" !="=&quot;undefined&quot;)" if(r.obj.id="=obj.data.id)" if(obj.group_by)="" obj.linegroup[r.d[obj.group_by]]="=" obj.linegroup[r.d[obj.group_by]].push(r.d.i);="" if(obj.map.projection)="" obj.linedata[r.d.i]="[];" latlonarray="obj.lineLatLons(r.d);" for(var="" x="" in="" latlonarray)="" xy="obj.map.projection([latLonArray[x][1],latLonArray[x][0]]);" obj.linedata[r.d.i].push({x:xy[0],y:xy[1]});="" this.load="function(r)" switch(obj.map.maptype)="" case="" "svg":="" obj.projection="map.projection;" break;="" "gmap":="" if(d3.select("#"+obj.group_id)[0][0]="=" null)="" obj.group="obj.map.addGroup(obj.group_id,obj.map.stage);" obj.path="d3.geo.path().projection(obj.projection);" if(debug)="" console.log("showing="" line="" data");="" obj.pathlayer="obj.map.addGroup(obj.pathgroup+&quot;_&quot;+obj.id,map[obj.svg]);" obj.pathlayer.classed(obj.pathgroup,true);="" obj.linepaths="[];" obj.line="d3.svg.line()" .x(function(d)="" d.x})="" .y(function(d)="" d.y})="" .interpolate(obj.interpolate);="" obj.data.data)="" d="obj.data.data[x];" console.log(obj.linedata[x]);="" obj.linepaths[d.i]="obj.pathlayer.append(&quot;path&quot;)" .datum(d,function(d)="" "d_"+obj.data.id+"_"+d.i;="" })="" bind="" data="" to="" object?="" .attr("id",="" "line_"+obj.id+"_"+d.i)="" .attr("i",="" d.i)="" .attr("class","line_"+obj.id)="" .classed("linegroup_"+obj.id+"_"+(typeof="" obj.group_by="=&quot;undefined&quot;?&quot;&quot;:obj.data.data[x][obj.group_by]),typeof" .attr("d",="" obj.line(obj.linedata[x]))="" .attr("stroke",="" function(d)="" obj.pathstrokecolor(obj.data.data[x],x,this);="" .attr("stroke-width",="" obj.pathstrokewidth(obj.data.data[x],x,this);="" .on("mouseover",="" function(d){="" obj.pathmouseover(this.id,this);="" what="" do="" when="" the="" mouse="" goes="" over="" .on("mouseout",="" obj.pathmouseout(this.id,this);="" is="" no="" longer="" if(obj.autosort)="" obj.sortbystrokewidth(obj.pathstrokewidth);="" if(obj.geojson)="" if(obj.map.maptype="=&quot;GMap&quot;)" console.log("gmap="" yet="" supported="" for="" geojson="" objects");="" false;="" d3.json(obj.geojson,="" function(error,="" world)="" if(debug||obj.debug)="" console.log("loading="" ("+(obj.geojson)+")");="" obj.topology="world;" console.log("geojson="" type:"="" +world.type);="" switch(world.type)="" "featurecollection":="" d3.select("#"+obj.group_id).selectall(obj.target)="" .data(world.features)="" .enter()="" .append(obj.target)="" .attr("class",obj.target_class)="" .attr("id",function(d,i)="" obj.setid="=" "function")="" obj.setid(d);="" "id_"+obj.id+"_"+i;="" function(d,="" i)="" d.i;="" same="" as="" above="" but="" just="" a="" number="" --="" sometime="" useful="" have="" access="" which="" .each(function(d,i)="" obj.each="=&quot;function&quot;)" obj.each(d,i,this);="" d.geometry.coordinates="" i="" d.geometry.coordinates)="" ii="" d.geometry.coordinates[i])="" lat="d.geometry.coordinates[i][1];" lon="d.geometry.coordinates[i][0];" obj.map.bounds.extend(r.d._latlng="new" google.maps.latlng(lat,="" lon));="" obj.map.bounds.extend(new="" obj.path)="" obj.mouseover_func(d,this)="" .on("mousemove",="" obj.mousemove_func(d,this)="" obj.mouseout_func(d,this);="" .on("click",="" obj.mouseclick_func(d,this);="" clicked="" d3vizobj.is_loaded(obj);="" this.mouseover_func="function(d,shape)" obj.mouseover_caption="=" d3.select("#"+obj.caption_id).html(obj.mouseover_caption(d,shape));="" if(obj.mouseover_caption)="" d3.select("#"+obj.caption_id).html(obj.mouseover_caption);="" obj.mouseover="=" obj.mouseover(d,shape);="" this.mousemove_func="function(d,shape)" obj.mousemove="=" obj.mousemove(d,shape);="" this.mouseout_func="function(d,shape)" d3.select("#"+obj.caption_id).html("");="" obj.mouseout="=" obj.mouseout(d,shape);="" this.mouseclick_func="function(d,shape)" obj.mouseclick="=" this.pathstrokewidth="function(d,i)" obj.strokewidth="=" 1;="" obj.strokewidth(d,i);="" obj.strokewidth;="" this.pathstrokecolor="function(d,i)" obj.strokecolor="=" "#000";="" obj.strokecolor(d,i);="" obj.strokecolor;="" this.pathmouseover="function(path_id,shape)" d3.select(shape).classed("mouseover",true);="" obj.mouseover(d,i,obj,path_id,shape);="" obj.map.tooltip.html(obj.mouseover_caption(d,i,obj,path_id,shape));="" obj.map.tooltip.html(obj.mouseover_caption,shape);="" this.pathmouseout="function(path_id,shape)" d3.select(shape).classed("mouseover",false);="" obj.mouseout(d,i,obj,path_id,shape);="" this.sortbystrokewidth="function(strokefunc)" sorts="" so="" that="" smallest="" radius="" always="" on="" top="" obj.map.svg.selectall(".line_"+obj.id)="" each="" of="" paths...="" .sort(function="" (a,b)="" resort="" bigger="" circles="" bottom="" strokefunc(b)="" -="" strokefunc(a);="" d3vizobj.addhook("run_on_each_data_row",this.processdata);="" d3vizobj.addhook("run_after_data_and_map_loaded",this.load);="" svgpath.prototype="new" d3viz(svgpath);="" if(window.debug)="" console.log("svgpath="" script="" loaded");="" text_controls.js="" *="" text="" controls="" by="" alex="" wellerstein,="" v.1.0="" constructor="" allows="" few="" types="" additions="" d3viz="" projects.="" do:="" data-driven="" captions="" (both="" viz="" objects="" choropleth="" forms?)="" interactivity="" (mouseover,="" etc.)="" textcaptions(options)="" options.load!="undefined" options.load="=" false)="" aborted="" obj.angle="setdefault(obj.angle,0);" obj.xoffset="setdefault(obj.xoffset,0);" obj.yoffset="setdefault(obj.yoffset,0);" obj.fontsize="setdefault(obj.fontsize,&quot;&quot;);" obj.class="setdefault(obj.class,&quot;caption&quot;);" "captions_"+obj.id);="" this.textcaptionsrun="function()" obj.viz="=" if(undef(obj.data))="" if(!undef(obj.viz))="" console.log("starting="" captions");="" obj.caption_count="0;" obj.captions="" "undefined"="" obj.captions.length="">0) {
		...
		function(data_field,filter_value,d) {
			if(typeof filter_value == "function") {
				...
				if(min===max) { //exact match
					...
					if(+data_field<max) {="" return="" true;="" }="" else="" false;="" if(max="==false)" if="" it="" is="" a="" "more="" than"="" situation="" if(+data_field="">min) {
						return true;
					} else {
						return false;
					}
				} else { //if it is a "between" situation
					if((+data_field&gt;min)&amp;&amp;(+data_field<max)) {="" return="" true;="" }="" else="" false;="" },="" ];="" this.datafilterbuildselect="function()" if(obj.selector_id)="" if(debug||obj.debug)="" console.log("building="" the="" text="" filter="" menu");="" var="" insert="document.getElementById(obj.selector_id);" if(!insert)="" console.log("could="" not="" find="" html="" entity="" with="" id="" '"+obj.selector_id);="" div="document.createElement(&quot;div&quot;);" div.id="dataFilter_" +obj.id;="" div.class="dataFilter" ;="" cap="document.createElement(&quot;span&quot;);" if(obj.selector_caption)="" cap_content="document.createTextNode(obj.selector_caption);" cap.appendchild(cap_content);="" cap.id="dataFilter_" +obj.id+"_label";="" div.appendchild(cap);="" sel="document.createElement(&quot;select&quot;);" sel.onchange="function()" obj.filter(obj.selectorvalue(this.value))="" };="" for(i="" in="" obj.selector_options)="" opt="document.createElement(&quot;option&quot;);" opt.text="obj.selector_options[i][0];" opt.value="i;" if(i="=obj.selector_selected)" opt.selected="true;" sel.add(opt);="" div.appendchild(sel);="" insert.appendchild(div);="" this.selectorvalue="function(selector_index)" obj.selector_selected="selector_index;" val="obj.selector_options[selector_index][1];" val;="" this.filter="function(val)" d3.selectall(obj.filter_target)="" select="" all="" data="" by="" class="" given="" .each(function(d)="" baseclass="toggleClass(d3.select(this).attr(&quot;class&quot;),obj.found_class,false);" if(val="==false)" if="" value="" is="" equal="" to="" false="" d3.select(this).attr("class",="" baseclass);="" just="" set="" default="" ("all")="" return;="" t="d[obj.filter_field];" if(typeof="" obj.before_comparing="=" "function")="" run="" "before_comparing"="" function="" on="" we="" need="" "string")="" if(obj.case_insensitive)="" if(obj.filterfunction[obj.filter_type](t,val,d))="" obj.filter_function="=" obj.filter_function(this,true);="" obj.toggleclass(baseclass,obj.found_class,true));="" behavior="" change="" obj.filter_function(this,false);="" obj.toggleclass(baseclass,obj.lacks_class,true));="" })="" toggles="" a="" or="" out="" of="" typical="" list="" this="" similar="" d3's="" .classed()="" but="" can="" be="" used="" within="" attr("class")="" which="" easier="" for="" lot="" cases="" it="" works="" understanding="" that="" classes="" are="" lists="" separated="" spaces,="" so="" converts="" an="" array,="" parses="" recombines="" into="" string="" this.toggleclass="function(classlist,classname,toggle)" ");="" if(toggle)="" if(classes.indexof(classname)="=-1)" classes.push(classname);="">-1) {
				classes.splice(classes.indexOf(classname),1);
			}
		}
		return classes.join(" ");
			...
			.classed("data_in_range",function(d) { //change the class based on the field
				if(value==0) { //if "all" is selected
					...
					if(min===max) { //exact match
						...
						if(+d[obj.range_filter_field]<max) {="" return="" true;="" }="" else="" false;="" if(max="==false)" if="" it="" is="" a="" "more="" than"="" situation="" if(+d[obj.range_filter_field]="">min) {
							return true;
						} else {
							return false;
						}
					} else { //if it is a "between" situation
						if((+d[obj.range_filter_field]&gt;min)&amp;&amp;(+d[obj.range_filter_field]<max)) {="" return="" true;="" }="" else="" false;="" })="" .classed("data_out_range",function(d)="" change="" the="" class="" based="" on="" field="" if(value="=0)" if="" "all"="" is="" selected="" just="" make="" it="" a="" regular="" dot="" but="" something="" var="" min="obj.range_filter_options[value][1]()[0];" max="obj.range_filter_options[value][1]()[1];" if(min="==max)" exact="" match="" if(+d[obj.range_filter_field]="=min)" (min="==false)" "less="" than"="" situation="" if(+d[obj.range_filter_field]<max)="" if(max="==false)" "more="">min) {
							return false;
						} else {
							return true;
						}
					} else { //if it is a "between" situation
						if((+d[obj.range_filter_field]&gt;min)&amp;&amp;(+d[obj.range_filter_field]<max)) {="" return="" false;="" }="" else="" true;="" })="" ;="" d3vizobj.addhook("run_before_map_loaded",this.rangefilterbuildselect);="" filterrange.prototype="new" d3viz(filterrange);="" if(window.debug)="" console.log("filter="" by="" range="" script="" loaded");="" load="" mouse_zoomer.js="" *="" mouse="" zoomer="" --="" makes="" it="" so="" that="" you="" can="" set="" certain="" objects="" if="" click="" on="" them,="" will="" zoom="" in="" out="" them.="" v.="" 2.0="" to="" use="" first="" select="" which="" (by="" class,="" element,="" or="" id)="" attach="" the="" events="" apply="" event="" class="" element="" id="" function="" mousezoomer(options)="" var="" obj="this;" obj.loadoptions(options);="" if(obj.map.maptype!="SVG" )="" if(debug||obj.debug)="" console.log("map="" only="" works="" svg="" maps="" not="" loading");="" d3vizobj.controls.push(obj);="" obj.id="d3VizObj.controls.length-1;" if(typeof="" obj.click_event="=&quot;undefined&quot;)" obj.zoom_factor="=&quot;undefined&quot;)" obj.positions_zoom_factor="=&quot;undefined&quot;)" obj.zoom_transition_speed="=&quot;undefined&quot;)" obj.preserve_strokes="=" "undefined")="" no="" longer="" necessary="" obj.centered="null;" obj.strokes="[];" obj.svg="=&quot;undefined&quot;)" this.addzoomevents="function()" console.log("attaching="" events");="" for(var="" i="" obj.zoom_if_clicked)="" "+obj.zoom_if_clicked[i]);="" obj.map[obj.svg].selectall(obj.zoom_if_clicked[i])="" .on(obj.click_event,="" obj.zoom)="" let="" take="" clicks="" console.log("done="" attaching");="" if(obj.preserve_strokes.length)="" this.preservestrokes="function()" console.log("zoomer:="" preserving="" strokes");="" obj.preserve_strokes)="" if((typeof="" window[obj.preserve_strokes[i]]="" !="undefined" )||(typeof="" obj.preserve_strokes[i]="" )||(obj.map[obj.svg].selectall(obj.preserve_strokes[i]).length="">0)) {
				   try {
...
</max))></max))></max)></max))></max)></len;></heatmapjs@patrick-wied.at></min)></current)></obj.date_precision)></body></html>
